...
(function(){
  ...
  function parseRow(text){
    ...
    const valor=parseMoney(t),dates=[...t.matchAll(dateRe)].map(x=>x[0]);
    ...
    if(bankMatch){const pre=clean(t.slice(0,bankMatch.index));const m=pre.match(/([A-ZÁÉÍÓÚÑ][A-ZÁÉÍÓÚÑ0-9 .&'-]+?)(?=\s+\(\d+\))/i);if(m)titular=clean(m[1])}
    if(titular==='SIN INFORMAR'){const m=t.match(/([A-ZÁÉÍÓÚÑ][A-ZÁÉÍÓÚÑ0-9 .&'-]+?\s+(?:S\.A\.|S\.R\.L\.|E\.A\.S\.|S\.A\.E\.))/i);if(m)titular=clean(m[1])}
    let cuenta='';
    if(bankMatch){const after=t.slice(bankMatch.index+bankMatch[0].length);const cm=after.match(/(?:^|\s)(\d{4,12})(?=\s+\d{1,2}\/)/);if(cm)cuenta=cm[1]}
  ...
  async function extract(file){
    ...
    for(let p=1;p<=pdf.numPages;p++){
      ...
      const rows=[];
      for(const it of items){let row=rows.find(r=>Math.abs(r.y-it.y)<7);if(!row){row={y:it.y,items:[]};rows.push(row)}row.items.push(it)}
  ...
  async function importPdf(file){
    ...
    try{
      ...
      // Si una importación quedó creada pero sin documentos por un error anterior,
      // reutilizarla. Esto evita bloquear la recuperación con "PDF ya importado".
      let {data:imp,error:ie}=await sb.from('cheque_importaciones').select('id').eq('hash_archivo',h).maybeSingle();
      if(ie)throw ie;
      if(!imp){
        const r=await sb.from('cheque_importaciones').insert({tipo:'DEVUELTO',nombre_archivo:file.name,fecha_base:new Date().toISOString().slice(0,10),hash_archivo:h}).select('id').single();
        if(r.error)throw r.error;imp=r.data;
      }
      const {data:existing,error:ee}=await sb.from('cheque_documentos').select('numero_cheque,valor,fecha_diferida').eq('importacion_id',imp.id).eq('tipo','DEVUELTO');
      if(ee)throw ee;
      const existingKeys=new Set((existing||[]).map(x=>`${x.numero_cheque}|${Number(x.valor||0)}|${x.fecha_diferida||''}`));
      const nuevos=rows.filter(r=>!existingKeys.has(`${r.numero_cheque}|${Number(r.valor||0)}|${r.fecha_diferida||''}`));
