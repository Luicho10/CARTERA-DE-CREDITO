...
(function(){
  ...
  // MISMO MÉTODO QUE 1. IMPORTAR PDF:
  // PDF.js -> items -> agrupación por coordenada Y -> orden por X -> texto reconstruido.
  ...
  function parseDataparCheques(text){
    ...
    // O relatório pode informar a situação como DEVUELTO e o movimento como
    // DEVUELTO/DEVOLVIDO. O texto real de Datapar testado contém DEVUELTO.
    if(!/SITUACI[ÓO]N\s*:\s*DEVUELTO/i.test(t))return [];
    if(!/CHEQUE\s+RECHAZADO/i.test(t))return [];

    // Estrutura real observada no PDF:
    // Nº cheque + titular + (código) + banco + conta + emissão + recep. + diferid.
    // + valor + moneda + CHEQUE RECHAZADO + ... + DEVUELTO.
    // O espaçamento puede desaparecer entre cuenta/fecha y fecha/monto.
    const out=[];
    const re=/(\d{5,8})([A-ZÁÉÍÓÚÑ][A-ZÁÉÍÓÚÑ0-9 .&'\-]*?)(?=\s*\(\d+\)\s+[A-ZÁÉÍÓÚÑ0-9 .&'\-]*?BANK)\s*\((\d+)\)\s*([A-ZÁÉÍÓÚÑ0-9 .&'\-]*?BANK)\s*(C\d+)(\d{1,2}\/\d{1,2}\/\d{2,4})\s*(\d{1,2}\/\d{1,2}\/\d{2,4})\s*(\d{1,2}\/\d{1,2}\/\d{2,4})\s*([\d.]+,\d{2})\s*(US\$|USD|GS\$|GS)\s+CHEQUE\s+RECHAZADO\s+.*?DEVUELTO/i;
    const m=t.match(re);
    if(!m)return [];

    const valor=parseMoney(m[9]);
    if(!valor)return [];
    ...
    out.push({
      ...
      moneda:/US\$|USD/i.test(m[10])?'USD':'GS',
      valor,
  ...
  async function importPdf(file){
    try{
      ...
      if(!rows.length){$('chequeStatus').textContent='El PDF fue leído, pero no se encontró un cheque DEVUELTO con la estructura Datapar. No se guardó información.';return}
